...
class Diklat extends BaseController
{
        ...
        if (session()->get("logged_dosen")) {
            ...
            if (strlen($pro->foto) > 0) {
                if (file_exists($this->modul->getPrivatePath() . $pro->foto)) {
                    $def_foto = base_url('diklat/showimg/' . esc($pro->foto));
                }
            }
            $data['foto'] = $def_foto;
    ...
    public function ajaxlist()
    {
        if (session()->get("logged_dosen")) {
            ...
            $data = array();
            $no = 1;
            $list = $this->mcustom->getDynamicData(false, ['*', 'date_format(tgl_mulai, "%d-%m-%Y") as tgl_mulaif', 'date_format(tgl_selesai, "%d-%m-%Y") as tgl_selesaif'], 'diklat', [], ['idusers' => $idusers], [], [], [], [], null, null, ['created_at' => 'ASC']);
            foreach ($list as $row) {
                $val = array();
                $val[] = $no;
                $val[] = esc($row->namadiklat);
                $val[] = esc($row->jenisdiklat);
                $val[] = esc($row->penyelengara);
                $val[] = esc($row->tahun_selenggara);
                $val[] = '<div style="text-align:center; width:100%;"><div class="btn-group" role="group">'
                    . '<button type="button" class="btn btn-xs btn-primary btn-fw" onclick="ganti(' . "'" . $row->iddiklat . "'" . ')"><i class="fa fa-fw fa-pencil"></i></button>'
                    . '<button type="button" class="btn btn-xs btn-danger btn-fw" onclick="hapus(' . "'" . $row->iddiklat . "'" . ',' . "'" . $no . "'" . ')"><i class="fa fa-fw fa-trash"></i></button>'
                    . '</div></div>';
                $data[] = $val;
