...
class Pangkat extends BaseController
{
        ...
        if (session()->get("logged_admin")) {
            ...
            if (strlen($pro->foto) > 0) {
                if (file_exists($this->modul->getPrivatePath() . $pro->foto)) {
                    $def_foto = base_url('pangkat/showimg/' . esc($pro->foto));
                }
            }
            $data['foto'] = $def_foto;
    ...
    public function ajaxlist()
    {
        if (session()->get("logged_admin")) {
            $data = array();
            $no = 1;
            $list = $this->mcustom->getDynamicData(false, [], 'pangkat', [], [], [], [], [], [], null, null, ['created_at' => 'ASC']);
            foreach ($list as $row) {
                $val = array();
                $val[] = $no;
                $val[] = esc($row->nama_pangkat);
                $val[] = '<div style="text-align:center; width:100%;"><div class="btn-group" role="group">'
                    . '<button type="button" class="btn btn-xs btn-primary btn-fw" onclick="ganti(' . "'" . $row->idpangkat . "'" . ')"><i class="fa fa-fw fa-pencil"></i></button>'
                    . '<button type="button" class="btn btn-xs btn-danger btn-fw" onclick="hapus(' . "'" . $row->idpangkat . "'" . ',' . "'" . $row->nama_pangkat . "'" . ')"><i class="fa fa-fw fa-trash"></i></button>'
                    . '</div></div>';
                $data[] = $val;
    ...
    public function ajax_add()
    {
        if (session()->get("logged_admin")) {
            $data = array(
                'idpangkat' => Uuid::uuid4()->toString(),
                'nama_pangkat' => esc($this->request->getPost('nama')),
                'created_at' => $this->modul->TanggalWaktu(),
                'updated_at' => $this->modul->TanggalWaktu()
            );
            $simpan = $this->mcustom->tambah("pangkat", $data);
            if ($simpan == 1) {
    ...
    {
        if (session()->get("logged_admin")) {
            $kond['idpangkat'] = esc($this->request->getUri()->getSegment(3));
            $data = $this->mcustom->get_by_id("pangkat", $kond);
    ...
    public function ajax_edit()
    {
        if (session()->get("logged_admin")) {
            ...
            $kond['idpangkat'] = esc($this->request->getPost('kode'));
            $simpan = $this->mcustom->ganti("pangkat", $data, $kond);
            if ($simpan == 1) {
    ...
    public function hapus()
    {
        if (session()->get("logged_admin")) {
            $kond['idpangkat'] = esc($this->request->getUri()->getSegment(3));
            $hapus = $this->mcustom->hapus("pangkat", $kond);
            if ($hapus == 1) {
