...
class Jabatan extends BaseController
{
        ...
        if (session()->get("logged_admin")) {
            ...
            if (strlen($pro->foto) > 0) {
                if (file_exists($this->modul->getPrivatePath() . $pro->foto)) {
                    $def_foto = base_url('jabatan/showimg/' . esc($pro->foto));
                }
            }
            $data['foto'] = $def_foto;
    ...
    public function ajaxlist()
    {
        if (session()->get("logged_admin")) {
            $data = array();
            $no = 1;
            $list = $this->mcustom->getDynamicData(false, [], 'jabatan', [], [], [], [], [], [], null, null, ['created_at' => 'ASC']);
            foreach ($list as $row) {
                $val = array();
                $val[] = $no;
                $val[] = esc($row->nama_jabatan);
                $val[] = '<div style="text-align:center; width:100%;"><div class="btn-group" role="group">'
                    . '<button type="button" class="btn btn-xs btn-primary btn-fw" onclick="ganti(' . "'" . $row->idjabatan . "'" . ')"><i class="fa fa-fw fa-pencil"></i></button>'
                    . '<button type="button" class="btn btn-xs btn-danger btn-fw" onclick="hapus(' . "'" . $row->idjabatan . "'" . ',' . "'" . $row->nama_jabatan . "'" . ')"><i class="fa fa-fw fa-trash"></i></button>'
                    . '</div></div>';
                $data[] = $val;
    ...
    public function ajax_add()
    {
        if (session()->get("logged_admin")) {
            $data = array(
                'idjabatan' => Uuid::uuid4()->toString(),
                'nama_jabatan' => esc($this->request->getPost('nama')),
                'created_at' => $this->modul->TanggalWaktu(),
                'updated_at' => $this->modul->TanggalWaktu()
            );
            $simpan = $this->mcustom->tambah("jabatan", $data);
            if ($simpan == 1) {
    ...
    {
        if (session()->get("logged_admin")) {
            $kond['idjabatan'] = esc($this->request->getUri()->getSegment(3));
            $data = $this->mcustom->get_by_id("jabatan", $kond);
    ...
    public function ajax_edit()
    {
        if (session()->get("logged_admin")) {
            ...
            $kond['idjabatan'] = esc($this->request->getPost('kode'));
            $simpan = $this->mcustom->ganti("jabatan", $data, $kond);
            if ($simpan == 1) {
    ...
    public function hapus()
    {
        if (session()->get("logged_admin")) {
            $kond['idjabatan'] = esc($this->request->getUri()->getSegment(3));
            $hapus = $this->mcustom->hapus("jabatan", $kond);
            if ($hapus == 1) {
