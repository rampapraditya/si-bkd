...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        table = $('#tb').DataTable({
            ajax: "<?php echo base_url('fakultas/ajaxlist'); ?>",
            ordering: false
        });
    });
    ...
    function save() {
        ...
        if (nama === '') {
            ...
            var url = "";
            if (save_method === 'add') {
                url = "<?php echo base_url('fakultas/ajax_add'); ?>";
            } else {
                url = "<?php echo base_url('fakultas/ajax_edit'); ?>";
            }
    ...
    function hapus(id, nama) {
        iziToast.show({
            ...
            title: 'Konfirmasi',
            message: 'Apakah yakin menghapus fakultas ' + nama + ' ?',
            ...
            buttons: [
                [
                    ...
                    function (instance, toast) {
                        ...
                        $.ajax({
                            url: "<?php echo base_url('fakultas/hapus/'); ?>" + id,
                            type: "GET",
                            dataType: "JSON",
    ...
    function ganti(id) {
        ...
        $('#form')[0].reset();
        $('#modal_form').modal('show');
        $('.modal-title').text('Ganti fakultas');
        $.ajax({
            url: "<?php echo base_url('fakultas/show/'); ?>" + id,
            type: "GET",
            dataType: "JSON",
            success: function (data) {
                $('[name="kode"]').val(data.idfakultas);
                $('[name="nama"]').val(data.namafakultas);
            }, error: function (jqXHR, textStatus, errorThrown) {
                iziToast.error({
                    title: 'Error',
                    message: "Error json " + errorThrown,
    ...
    function detil(id){
        window.location.href = "<?php echo base_url('fakultas/jurusan/') ?>" + id;
    }
