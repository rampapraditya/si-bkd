...
<div class="content-wrapper">
    <section class="content-header">
        <h1>Anggota Profesi <small>View data anggota profesi</small> </h1>
        <ol class="breadcrumb">
            <li><a href="<?php echo base_url('dashboard'); ?>"> Beranda</a></li>
            <li class="active">Anggota Profesi</li>
        </ol>
    </section>
                        ...
                        <table id="tb" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th>Peran /<br>Kedudukan</th>
                                    <th>Mulai<br>Keanggotaan</th>
                                    <th>Selesai<br>Keanggotaan</th>
                                    <th>Instansi<br>Profesi</th>
            ...
            <div class="modal-body">
                <form id="form" class="form-horizontal">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-8">
                            <input type="text" class="form-control" id="nama_org" name="nama_org" autocomplete="off">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-sm-4 control-label">Peran / Kedudukan</label>
                        <div class="col-sm-8">
                            <input type="text" class="form-control" id="peran" name="peran" autocomplete="off">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-sm-4 control-label">Mulai Keanggotaan</label>
                        <div class="col-sm-8">
                        <input type="date" class="form-control" id="mulai" name="mulai" autocomplete="off" value="<?php echo $curdate; ?>">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-sm-4 control-label">Selesai Keanggotaan</label>
                        <div class="col-sm-8">
                            <input type="date" class="form-control" id="selesai" name="selesai" autocomplete="off" value="<?php echo $curdate; ?>">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-sm-4 control-label">Instansi Profesi</label>
                        <div class="col-sm-8">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        table = $('#tb').DataTable({
            ajax: "<?php echo base_url('anggota-profesi/ajaxlist'); ?>",
            ordering: false
        });
    });
    ...
    function add() {
        ...
        $('.modal-title').text('Tambah anggota profesi');
    }

    function save() {
        var kode = document.getElementById('kode').value;
        var nama_org = document.getElementById('nama_org').value;
        ...
        if (nama_org === '') {
            ...
            var url = "";
            if (save_method === 'add') {
                url = "<?php echo base_url('anggota-profesi/ajax_add'); ?>";
            } else {
                url = "<?php echo base_url('anggota-profesi/ajax_edit'); ?>";
            }
            
            var form_data = new FormData();
            form_data.append('kode', kode);
            form_data.append('nama_org', nama_org);
    ...
    function hapus(id, nama) {
        iziToast.show({
            ...
            title: 'Konfirmasi',
            message: 'Apakah yakin menghapus anggota profesi nomor ' + nama + ' ?',
            ...
            buttons: [
                [
                    ...
                    function (instance, toast) {
                        ...
                        $.ajax({
                            url: "<?php echo base_url('anggota-profesi/hapus/'); ?>" + id,
                            type: "GET",
                            dataType: "JSON",
    ...
    function ganti(id) {
        ...
        $('#form')[0].reset();
        $('#modal_form').modal('show');
        $('.modal-title').text('Ganti anggota profesi');
        $.ajax({
            url: "<?php echo base_url('anggota-profesi/show/'); ?>" + id,
            type: "GET",
            dataType: "JSON",
            success: function (data) {
                $('[name="kode"]').val(data.idang_profesi);
                $('[name="nama_org"]').val(data.nama_org);
                $('[name="peran"]').val(data.peran);
                $('[name="mulai"]').val(data.mulai);
                $('[name="selesai"]').val(data.selesai);
                $('[name="ins"]').val(data.instansi_profesi);
